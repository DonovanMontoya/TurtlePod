...
Ad detection can use OpenAI, Apple On-Device, or Jev 1.13 through TypeSafe or OpenRouter. Transcription can use OpenAI, Local Whisper, or **Apple Speech** on iOS 26 and later. Apple Speech transcribes downloaded audio on device in English; its model may already be installed or may download on first use. Settings shows the current asset status. Pair either local transcriber with Jev to keep hosted model costs to classification only; each Jev route requires its own API key in Settings. Jev sees transcript text; the app computes skip times from audio transcription cues and refines candidate passages cue by cue. The MFM Minisode 504 audio transcript exercises Jev routing and timestamp handling with a mocked response in the portable tests. Live Jev accuracy and end-to-end cost still need a service key and a labeled evaluation set.
